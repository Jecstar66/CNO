...
def main():
    ...
    # Base Params
    parser.add_argument("--model", type=str, default="sd15", choices=["sd15", "sdxl"], help="Choose SD model version")
    ...
    parser.add_argument("--b_size", type=int, default=4, help="Batch size (CNO requires > 1)")
    parser.add_argument("--num_samples", type=int, default=10000)
    
    # CNO (InfoNCE) Params
    parser.add_argument("--use_cno", action='store_true', default=True)
    parser.add_argument("--i_opt_iter", type=int, default=3)
    parser.add_argument("--i_opt_lr", type=float, default=1e-3)
    parser.add_argument("--window_size", type=int, default=1, choices=[1,2,4,8,16,32,64])
    ...
    if args.model == "sdxl":
        from latent_sdxl_cno import get_solver
        model_key = "stabilityai/stable-diffusion-xl-base-1.0"
        print("Using SDXL for generation.")
    else:
        from latent_diffusion_cno import get_solver
        model_key = "botp/stable-diffusion-v1-5" 
        print("Using SD1.5 for generation.")

    # Load Prompts
    text_list = []
    with open(args.prompt_dir, 'r') as f:
        for line in f:
            if line.strip(): text_list.append(line.strip())
